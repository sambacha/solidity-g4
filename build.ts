...
export const grammarFiles = ['SolidityParser.g4', 'SolidityLexer.g4'];
export const grammarRoot = 'src/parser';
    ...
    await Promise.all(
      grammarFiles.map(async (file) => {
        const res = await fetch(
          `https://raw.githubusercontent.com/ethereum/solidity/blob/develop/docs/grammar/${file}`,
        );
...
const writeFiles = (files: { [file: string]: string }) =>
  Promise.all(
    Object.entries(files).map(([file, data]) =>
      fs.writeFile(path.join(__dirname, grammarRoot, file), data),
    ),
  );
...
const writeParser = () => {
  ...
  return execCommand(
    `${binary} -visitor -o ${grammarRoot} -Xexact-output-dir ${grammarRoot}/SolidityLexer.g4 ${grammarRoot}/SolidityParser.g4`,
  );
};

const writeParserContexts = async () => {
  const listenerSource = await fs.readFile(
    path.join(__dirname, grammarRoot, 'SolidityParserListener.ts'),
    'utf-8',
  );
  ...
  await fs.writeFile(
    path.join(__dirname, grammarRoot, 'SolidityContexts.ts'),
    `export {${EOL}${exportList}} from './SolidityParser';`,
  );
};

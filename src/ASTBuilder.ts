...
import * as SP from './SolidityParser'

import { ParseOptions } from './types'
import * as AST from './ast-types'
import { ErrorNode } from 'antlr4ts/tree/ErrorNode'
require("./SolidityParser");
...
export class ASTBuilder
  ...
  public visitContractPart(ctx: ContractPartContext) {
  ...
  public visitContractDefinition(
    ...
    const node: AST.ContractDefinition = {
      ...
      baseContracts: ctx
        ...
        .map((x: SP.InheritanceSpecifierContext) => this.visitInheritanceSpecifier(x)),
      subNodes: ctx.contractPart().map((x: ParseTree) => this.visit(x)),
      kind,
    }
  ...
  public visitStateVariableDeclaration(
    ...
    if (overrideSpecifier.length === 0) {
      ...
      override = overrideSpecifier[0]
        .userDefinedTypeName()
        .map((x: any) => this.visitUserDefinedTypeName(x))
    }
  ...
  public visitEventDefinition(ctx: EventDefinitionContext) {
    const parameters = ctx
      .eventParameterList()
      .eventParameter()
      .map((paramCtx: ParserRuleContext) => {
  ...
  public visitFunctionDefinition(
    ...
    const modifiers = ctx
      .modifierList()
      .modifierInvocation()
      .map((mod: SP.ModifierInvocationContext) => this.visitModifierInvocation(mod))
  ...
  public visitEnumDefinition(
    ...
    const node: AST.EnumDefinition = {
      type: 'EnumDefinition',
      name: this._toText(ctx.identifier()),
      members: ctx.enumValue().map((x: any) => this.visitEnumValue(x)),
    }
  ...
  public visitPragmaDirective(
    ...
    if (versionContext?.children !== undefined) {
      value = versionContext.children.map((x: ParserRuleContext | ParseTree) => this._toText(x)).join(' ')
    }
  ...
  public visitInheritanceSpecifier(
    ...
    const args =
      exprList !== undefined
        ? exprList.expression().map((x: SP.ExpressionContext) => this.visitExpression(x))
        : []
  ...
  public visitModifierInvocation(
    ...
    let args: any[]
    if (exprList != null) {
      args = exprList.expression().map((x: ParseTree) => this.visit(x))
  ...
  public visitTypeNameExpression(
    ...
    let typeName: (AST.ElementaryTypeName & WithMeta) | (AST.UserDefinedTypeName & WithMeta)
  ...
  public visitFunctionTypeName(
    ...
    const parameterTypes = ctx
      ...
      .map((typeCtx: any) => this.visitFunctionTypeParameter(typeCtx))
  ...
  public visitFunctionCall(
    ...
    if (ctxArgsExpressionList) {
      args = ctxArgsExpressionList
        .expression()
        .map((exprCtx: SP.ExpressionContext) => this.visitExpression(exprCtx))
  ...
  public visitStructDefinition(
    ...
    const node: AST.StructDefinition = {
      ...
      members: ctx
        .variableDeclaration()
        .map((x: SP.VariableDeclarationContext) => this.visitVariableDeclaration(x)),
    }
  ...
  public visitHexLiteral(ctx: SP.HexLiteralContext) {
    const parts = ctx
      .HexLiteralFragment()
      .map((x: ParserRuleContext | ParseTree) => this._toText(x))
      .map((x: string) => x.substring(4, x.length - 1))
  ...
  public visitImportDirective(ctx: SP.ImportDirectiveContext) {
    ...
    if (ctx.importDeclaration().length > 0) {
      symbolAliases = ctx.importDeclaration().map((decl: { identifier: (arg0: number) => ParseTree; }) => {
      ...
      symbolAliasesIdentifiers = ctx.importDeclaration().map((decl: { identifier: (arg0: number) => SP.IdentifierContext; }) => {
  ...
  public visitAssemblyBlock(
    ...
    const operations = ctx
      .assemblyItem()
      .map((item: any) => this.visitAssemblyItem(item))
  ...
  public visitAssemblyCall(ctx: SP.AssemblyCallContext) {
    ...
    const args = ctx
      .assemblyExpression()
      .map((assemblyExpr: any) => this.visitAssemblyExpression(assemblyExpr))
  ...
  public visitAssemblySwitch(ctx: SP.AssemblySwitchContext) {
    const node: AST.AssemblySwitch = {
      ...
      cases: ctx.assemblyCase().map((c: any) => this.visitAssemblyCase(c)),
    }
  ...
  public visitAssemblyLocalDefinition(
    ...
    let names: (AST.Identifier & WithMeta)[] | (AST.AssemblyMemberAccess & WithMeta)[]
    if (ctxAssemblyIdentifierOrList.identifier()) {
      ...
      names = ctxAssemblyIdentifierOrList
        ...
        .identifier()!
        .map((x: SP.IdentifierContext) => this.visitIdentifier(x))
    }
